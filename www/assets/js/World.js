...
export class World {
    ...
    bulletWallHit(origin, hitPosition, item) {
        ...
        const result = this.#raycaster.intersectObject(this.getScene())
        if (result.length === 0) {
            return
        }

        const radius = (item.slot === Enum.InventorySlot.SLOT_PRIMARY || item.slot === Enum.InventorySlot.SLOT_SECONDARY) ? 1.5 : 1.1
        const hit = new THREE.Mesh(
            this.loadCache(`bulletHit-wc-${radius}`, () => new THREE.CylinderGeometry(radius, radius, 0.8, 8, 1)),
            new THREE.MeshBasicMaterial({color: new THREE.Color(`hsl(23, 24%, ${Math.random() * 18}%, 1)`)}),
        )
        hit.position.copy(rayHit)
        hit.lookAt(result[0].point)
        hit.rotateX(Utils.degreeToRadian(90))
        hit.translateY(result[0].distance)
