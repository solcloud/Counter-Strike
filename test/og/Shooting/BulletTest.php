...
class BulletTest extends BaseTestCase
{
    ...
    {
        $spawn = new Point(Player::gunHeightStand, 0, Player::gunHeightStand);
        $playerCommands = [
        ...
        $game->getWorld()->addWall(new Wall(new Point(0, $spawn->y, $spawn->z + Player::gunHeightStand + 1), true, 2 * $spawn->x));
        $game->getWorld()->addWall(new Wall(new Point($spawn->x + Player::gunHeightStand + 1, $spawn->y, 0), false, 2 * $spawn->z));
        $game->getWorld()->addFloor(new Floor(new Point(0, Player::gunHeightStand * 2, 0), PHP_INT_MAX, PHP_INT_MAX));
        ...
        $this->assertGreaterThanOrEqual(Player::gunHeightStand, $result->getBullet()->getDistanceTraveled(), "Angles [{$angleHorizontal},{$angleVertical}]");
        $this->assertLessThan(sqrt(pow(Player::gunHeightStand, 2) + pow(Player::gunHeightStand, 2)), $result->getBullet()->getDistanceTraveled(), "Angles [{$angleHorizontal},{$angleVertical}]");
        if ($angleVertical < 0) {
            $this->assertLessThan($spawn->y + $player::gunHeightStand, $result->getBullet()->getPosition()->y, "Angles [{$angleHorizontal},{$angleVertical}]");
        } elseif ($angleVertical === 0) {
            $this->assertSame($spawn->y + $player::gunHeightStand, $result->getBullet()->getPosition()->y, "Angles [{$angleHorizontal},{$angleVertical}]");
        } else {
            $this->assertGreaterThan($spawn->y + $player::gunHeightStand, $result->getBullet()->getPosition()->y, "Angles [{$angleHorizontal},{$angleVertical}]");
        }
    ...
    public function testBulletHitFloor(): void
    {
        ...
        $maxDistance = (int)floor(sqrt(pow(Player::gunHeightStand, 2) + pow(Player::gunHeightStand, 2)));
        ...
        $this->assertSame(Player::gunHeightStand, $result->getBullet()->getDistanceTraveled());
